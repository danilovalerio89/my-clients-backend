...
describe("Testing PATCH method in /contacts", () => {
  ...
  test("Trying to update a contact ", async () => {
    ...
    expect(contactPatch.status).toEqual(200);
    expect(contactPatch.body.firstName).toEqual("Patched");
  });

  test("Trying to update a contact without credentials", async () => {
    const logIn = await request(app).post("/login").send(userLogin);
